...
float4	get_normal(__global t_primitive *obj, float4 point)
{
	...
	switch (obj->type)
	{
		...
		case CYLINDER:
			...
			break;
		case CONE:
			t = COS(obj->radius * M_PI / 180.0f);
			n = point - obj->position + (obj->direction * -DOT(point, obj->direction) / t * t);
			break;
		default:
			return (n);
	}
...
__kernel void	raytracer(							//main kernel, called for each ray
